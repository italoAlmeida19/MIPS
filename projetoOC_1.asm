.data
 ...
 .text 
	...
	#RODRIGO SOARES DE ASSIS 	  - 202176027
 	...
 	move $a2, $t1 #move o valor passado para a fun��o
 	jal inicializaVetor
	...
	#CHAMA ORDENA VETOR
 	...
 	imprime:  #fun��o de imprime vetor
 		move $t0, $a0  #Coloca o endere�o do vetor que veio por par�metro dentro de $t0
 		...
 		sll $t2, $a1, 2  #Coloca o tamanho do vetor em bytes dentro de $t2
 			while:
				...
				li $v0, 4       	# Espa�o entre elementos
				la $a0, newSpace        
				syscall
				
				addi $t0, $t0, 4      #anda 1 posi��o no vetor
	...
	zeraVetor:  #fun��o de zerar vetor
		move $t0, $a0  #guada primeira posi��o endere�o
		move $t1, $a1  #guarda ultima posi��o endere�o
		whileZera:
			bgt $t0, $t1, fim  #posicao inicial > posi��o final ? caso afirmativo pular para fim 
			sw $zero, 0($t0)   #coloca 0 na vet[atual]
			addi $t0, $t0, 4   #avan�a para proximo elemento do vetor 
	...
	numeroAleatorio:
		lw  $t3, 0($sp)  #acessa o 4 par�metro na pilha
		
		mul $t0, $a0, $a1 #realiza todas as opera��es
		add $t1, $t0, $a2
		div $t1, $a3
		mfhi $t2
		sub $t4,$t2, $t3
	...
	inicializaVetor:
		#Salvo na pilha todos os S que irei usar na fun��o e o ra
		addi $sp, $sp, -16	
		sw $s0, 0($sp)
		sw $s1, 4($sp) 
		sw $s2, 8($sp) 
		...
		#Recupera informa��es da pilha para poder retornar para quem chamou 
		lw $s0, 0($sp)
		lw $s1, 4($sp) 
		lw $s2, 8($sp) 
		lw $ra, 12($sp) 
		...
		prox:
			...
			#coloca o numero aleatorio na ultila posi��o do vetor 
			addi $s2,$s2, -1  #n - 1 
			sll $t4, $s2, 2   #n *4
			add $s1, $s1, $t4 #pulo pra ultima posi��o do vetor disponivel
			sw $s0, 0($s1)    #coloco o resultado do a0 nessa posi��o 
			sub  $s1, $s1, $t4 #volto o ponteiro do vetor pro inicio
			
			
			#Carrega parametros para chamar denovo a fun�a�
			move $a0, $s1  #vetor
			...
			add $v0, $s0, $t5   #soma numero aleatorio dessa chamada + resultado da recurs�o
